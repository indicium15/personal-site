...
<iframe src="{{'/'|relative_url}}assets/ureca2023/model-performance.html" width="100%" height="340px" frameborder="0" scrolling="no" style="margin: 12px 0;"></iframe>
<div style="text-align: center;"><em>Figures 5–7. Precision, Recall, and F1-Score by model and grip class. Use the dropdowns to switch metric and player dataset.</em></div>

As shown above, the Conv2D model consistently outperformed the RNN and LSTM architectures, achieving the highest F1-scores across all datasets. This suggests that convolutional layers are particularly effective at capturing the local spatial and temporal patterns in joint-based features.
